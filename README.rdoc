...
The last two run all levels for each NN to be tested, so each level is run in a separate thread
...
The Invigilator class evaluates the NN run in a level and is essentially the 'fitness function'.  As a separate class this means is can be changed mid evolution; halt execution of .train and paste in redefined Invigilator, the run .train again on the current training ground instance.
